...
#define PCNT_INPUT_UNIT     PCNT_UNIT_0
...
#define PCNT_INPUT_SIG_IO   21             // Pulse Input GPIO
...
static void pcnt_init_and_start(void)
{
    ...
    pcnt_config.unit           = PCNT_INPUT_UNIT;
    // What to do on the positive / negative edge of pulse input?
    pcnt_config.pos_mode       = PCNT_COUNT_INC;   // Count up on the positive edge
    pcnt_config.neg_mode       = PCNT_COUNT_DIS;   // Keep the counter value on the negative edge
    // What to do when control input is low or high?
    pcnt_config.lctrl_mode     = PCNT_MODE_KEEP; // Keep counting direction if low
    ...
    //pcnt_set_filter_value(PCNT_INPUT_UNIT, 100);
    //pcnt_filter_enable(PCNT_INPUT_UNIT);

    /* [Optional] Set threshold 0 and 1 values and enable events to watch */
    pcnt_set_event_value(PCNT_INPUT_UNIT, PCNT_EVT_THRES_1, PCNT_THRESH1_VAL);
    pcnt_event_enable(PCNT_INPUT_UNIT, PCNT_EVT_THRES_1);
    pcnt_set_event_value(PCNT_INPUT_UNIT, PCNT_EVT_THRES_0, PCNT_THRESH0_VAL);
    pcnt_event_enable(PCNT_INPUT_UNIT, PCNT_EVT_THRES_0);
    /* [Optional] Enable events on zero, maximum and minimum limit values */
    pcnt_event_enable(PCNT_INPUT_UNIT, PCNT_EVT_ZERO);
    pcnt_event_enable(PCNT_INPUT_UNIT, PCNT_EVT_H_LIM);
    pcnt_event_enable(PCNT_INPUT_UNIT, PCNT_EVT_L_LIM);

    /* Initialize PCNT's counter */
    pcnt_counter_pause(PCNT_INPUT_UNIT);
    pcnt_counter_clear(PCNT_INPUT_UNIT);
